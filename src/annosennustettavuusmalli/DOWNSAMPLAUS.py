    ...
    if not patients:
        ...
        for p in patients:
            ...
            folders = {
                ...
                "dose": os.path.join(DESTINATION_PATH, patient_name, "dose"),       # alkuperäinen dose
                "doseds": os.path.join(DESTINATION_PATH, patient_name, "doseds"),   # uusi downsampled/copy dose
            ...
            dose_files = [os.path.join(dose_src_folder, f) for f in os.listdir(dose_src_folder)
            ...
            if dose_files:
                try:
                    ...
                    if hasattr(ds_dose, "PixelSpacing"):
                        ds_dose.PixelSpacing = MultiValue(float, [float(x)*2 for x in ds_dose.PixelSpacing])
                except Exception as e:
            ...
            mask_files = [os.path.join(mask_src_folder, f) for f in os.listdir(mask_src_folder)
            ...
            for f in mask_files:
                try:
                    ...
                    warnings.warn(f"{patient_name}: Mask-tiedoston {os.path.basename(f)} käsittely epäonnistui: {e}")
            ...
            if ds_dose is not None and arr_dose_down is not None:
                try:
                    ...
                    warnings.warn(f"{patient_name}: RD-tiedoston tallennus doseds-kansioon epäonnistui: {e}")
