...
class CreateDistanceToPTV(tio.transforms.Transform):
    ...
    def apply_transform(self, subject: tio.Subject) -> tio.Subject:
        ...
        subject["distance_to_PTV"].set_data(
            distance_to_PTV / 400
        )  # 400 is an arbitrary number to approximately normalize the distances to [0, 1]
        return subject
...
class CreateInputMask(tio.transforms.Transform):
    ...
    def apply_transform(self, subject: tio.Subject) -> tio.Subject:
        ...
        final_mask[
            torch.isclose(rounded_mask[:, :, :, :], torch.tensor([-1], dtype=torch.int))
        ] = -1
