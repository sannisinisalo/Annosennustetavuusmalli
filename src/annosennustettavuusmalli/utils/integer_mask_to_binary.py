...
def integer_mask_to_binary(arr: np.ndarray | torch.Tensor, num_bits: int):
    ...
    if isinstance(arr, np.ndarray):
        ...
        bin_nums = (
            (arr.reshape(arr.shape + (1,)) & (2 ** np.arange(num_bits))) != 0
        ).astype(int)
        ...
        bin_nums = (
            (arr.reshape(arr.shape + (1,)) & (2 ** torch.arange(num_bits))) != 0
        ).int()
    else:
