...
from torchio.constants import DATA, LOCATION
    ...
    with torch.no_grad():
        for subject in dataset:
            ...
            loader = torch.utils.data.DataLoader(
                inference_sampler, batch_size=1, num_workers=4
            )  # Here num_workers can be > 0 as tio.Queue is not used
            aggregator = tio.inference.GridAggregator(inference_sampler)

            for patches_batch in loader:
                ct = patches_batch["ct"][DATA].to(dtype=torch.float, device=device)
                mask = patches_batch["mask"][DATA].to(dtype=torch.float, device=device)
                PTV_dist = patches_batch["distance_to_PTV"][DATA].to(
            ...
            binary_mask = integer_mask_to_binary(subject["original_mask"][DATA], 11)

            for structure, mask_number in structure_config.items():
                structure_mask = binary_mask[:, :, :, :, mask_number]
                structure_metrics = calculate_structure_metrics(
                    dose, pred_dose, structure_mask
                )
...
def calculate_structure_metrics(dose, pred_dose, structure_mask):
    ...
    return {
        "mean_pred_dose": mean_pred_dose.item(),
