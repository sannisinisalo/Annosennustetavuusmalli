    ...
    def __init__(
        ...
        self.convolution = nn.ModuleList(
            [
                nn.Conv3d(
                    ...
                    bias=False if self.is_bn else True,
                )
        ...
        if is_bn:
            self.batch_norm = nn.ModuleList(
                [nn.BatchNorm3d(num_features=out_channels) for i in range(conv_layers)]
            )
        ...
        for block_idx in range(self.num_blocks):
            for i in range(1):
                intermediate_channels = (
                    in_channels if block_idx == 0 and i == 0 else out_channels
                )
                block = nn.Sequential(
                    nn.Conv3d(
                        intermediate_channels,
                ...
                if self.is_bn:
                    block.add_module(
                        f"BN_{block_idx}_{i}", nn.BatchNorm3d(num_features=out_channels)
                    )
                block.add_module(
                    f"Act_{block_idx}_{i}", getattr(nn, act_func)(inplace=True)
                )
                self.blocks.add_module(
                    f"DepthwiseSeparableConvBlock_{block_idx}_{i}", block
                )
...
class SeparableBlock3d(nn.Module):
    def __init__(
        ...
        for i in range(self.conv_layers):
            in_channels = self.in_channels if i == 0 else self.out_channels
            self.layers.append(
                SeparableConvolution3d(in_channels, self.out_channels, self.kernel_size)
            )
