...
from .basic_blocks import ConvBlock, ResNetBlock
...
class SkipConnectionStack(nn.Module):
    def __init__(self, current_depth: int, total_depth: int, in_channels: List[int], pool_size: tuple, kernel_size: tuple, out_channels: int = 64, act_func: str = 'relu') -> None:
        ...
        self.convolution = nn.ModuleList([ConvBlock(in_channels = in_channels[i + 1] if i <= self.current_depth or i == (self.total_depth - 1) else self.filters*self.total_depth, 
                                                    out_channels = self.filters, 
                                                    conv_layers = 1, 
                                                    kernel_size = self.kernel_size, 
                                                    padding = 'same', 
                                                    act_func = self.act_func,
                                                    is_bn = True) for i in range(self.total_depth)])
        ...
        self.poolings = nn.ModuleList([nn.MaxPool3d(kernel_size = (self.pool_size[0]**abs(self.current_depth-i), 
                                                                   self.pool_size[1]**abs(self.current_depth-i), 
                                                                   self.pool_size[2]**abs(self.current_depth-i))) if i < self.current_depth 
                                       else nn.Upsample(scale_factor = (self.pool_size[0]**abs(self.current_depth-i), 
                                                                        self.pool_size[1]**abs(self.current_depth-i), 
                                                                        self.pool_size[2]**abs(self.current_depth-i)),
                                                        mode = 'trilinear') for i in range(self.total_depth)])
...
class DeepSupervisionOutput(nn.Module):
    def __init__(self, current_depth: int, total_depth: int, in_channels: int, pool_size: tuple, kernel_size: tuple, is_bn: bool = False, is_act: bool = True) -> None:
        ...
        self.upsampling = nn.Upsample(scale_factor = (self.pool_size[0]**self.current_depth,
                                                      self.pool_size[1]**self.current_depth,
                                                      self.pool_size[2]**self.current_depth),
                                      mode = 'trilinear')
        
        if is_act:
            self.act = nn.ReLU() # Not affected by act_func, as we don't want negative dose values
        
        if is_bn:
            self.bn = nn.BatchNorm3d(num_features = 1) # one because there's only one deep supervision channel per depth to output      
...
class UNet3plus_3d(nn.Module):
    def __init__(self, 
                 ...
                 conv_layers: int = 2, 
                 act_func: str = 'ReLU',
                 patch_size: tuple = (256, 256, 16)) -> None:
        ...
        if patch_size[2] > 1:
            if patch_size[2]%pooling_reduction != 0:
                raise ValueError("z-dimension of patch_size violates with selected pooling size and UNet depth")
            
        self.conv_layers = conv_layers
        self.filters = filters.copy() # Otherwise insertion will change the original list!
        ...
        self.conv_encoder = nn.ModuleList([ConvBlock(in_channels = self.filters[i], 
                                                     out_channels = self.filters[i+1], 
                                                     conv_layers = self.conv_layers, 
                                                     kernel_size = self.kernel_size, 
                                                     padding = 'same', 
                                                     act_func = self.act_func,
                                                     is_bn = True) for i in range(self.depth)])
        
        # This is a list of decoder convolution stacks
        self.conv_decoder = nn.ModuleList([ConvBlock(in_channels = self.skip_filters*self.depth, 
                                                     out_channels = self.skip_filters*self.depth, 
                                                     conv_layers = 1,
                                                     kernel_size = self.kernel_size, 
                                                     padding = 'same', 
                                                     act_func = self.act_func,
                                                     is_bn = True) for i in range(self.depth - 1)])
        
        # This is list of pooling modules used between encoder convolutions
        self.pooling_encoder = nn.ModuleList([nn.MaxPool3d(self.pool_size) for i in range(self.depth - 1)])
        
        # This is a list of skip connections. Inputs for these are encoder stacks, and outputs are decoder stacks.
        self.skip_conv_stacks = nn.ModuleList([SkipConnectionStack(current_depth = i, 
                                                                   total_depth = self.depth, 
                                                                   pool_size = self.pool_size,
                                                                   kernel_size = self.kernel_size,
                                                                   in_channels = self.filters, 
                                                                   out_channels = self.skip_filters, 
                                                                   act_func = self.act_func) for i in range(self.depth - 1)])
        ...
        self.deep_supervision_outputs = nn.ModuleList([DeepSupervisionOutput(in_channels = self.skip_filters*self.depth if i < (self.depth - 1) else self.filters[-1], 
                                                                             current_depth = i, 
                                                                             total_depth = self.depth, 
                                                                             pool_size = self.pool_size,
                                                                             kernel_size = self.kernel_size,
                                                                             is_bn = False, 
                                                                             is_act = True) for i in range(self.depth)])
 
    def forward(self, x: torch.Tensor) -> namedtuple:

        Output = namedtuple('Output', ['main_output'] + ['deep_supervision_{}'.format(i+1) for i in range(self.depth - 1)])
        ...
        for depth_iter in reversed(range(self.depth - 1)):
            skip_nodes.insert(0, self.skip_conv_stacks[depth_iter](encoder_nodes[:depth_iter+1] + decoder_nodes[:])) # List is mutable, so [:] prevents passing parameter by reference
            decoder_nodes.insert(0, self.conv_decoder[depth_iter](skip_nodes[0]))

        deep_supervision_nodes = []
        
        for i in range(self.depth):
            deep_supervision_nodes.append(self.deep_supervision_outputs[i](decoder_nodes[i]))
            
        return Output(*deep_supervision_nodes)