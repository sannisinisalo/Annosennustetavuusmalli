...
# Tämä hoidetaan AllPatients luokan avulla
...
def _find_dicom_by_modality(folder: Path, modality: str) -> List[Path]:
...
if __name__ == "__main__":
    ...
    patients = AllPatients(data_root=SOURCE_PATH)

    # Tämä hoidetaan AllPatients-luokassa, joten ei tarvitse erikseen tarkistaa onko potilaskansioita olemassa
    # if not patients:
    #     print("Ei löytynyt potilaskansioita (Patient*).")
    # else:

    for p in patients.sorted_by_number():
        patient_name = p.name
        print(f"Processing {patient_name}...")

        # Find DICOM files by modality
        ct_files = _find_dicom_by_modality(p.path, "CT")
        rd_files = _find_dicom_by_modality(p.path, "RTDOSE")
        rp_files = _find_dicom_by_modality(p.path, "RTPLAN")
        rs_files = _find_dicom_by_modality(p.path, "RTSTRUCT")
